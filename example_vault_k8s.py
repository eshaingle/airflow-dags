...
passing = KubernetesPodOperator(namespace='default',
				service_account_name="vault-auth",
                                image="alpine:3.7",
                                cmds=["sh", "-cx"],
                                arguments=["curl --request POST --data \'{\"jwt\": "'"$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)"'", \"role\": \"example\"}\' http://192.168.49.2:8200/v1/auth/kubernetes/login"],
